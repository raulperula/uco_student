...
{
	if(I._data == NULL){
		throw ImageException("Error en FImage(I): imagen no cargada previamente");
	}
...
FImage::FImage(int w, int h, int type, unsigned char c)
{
	...
	if((_type == GREY) || (_type == COLOR)){
		...
		throw ImageException("Error en FImage(w,h,type,c): imagen no cargada previamente");
	}
}
...
float FImage::operator[](int i) throw(ImageException)
{
	if((i < 0) || (i > _width*_height*_type)){
		throw ImageException("Error en operator[]: coordenada fuera de rango");
	}
