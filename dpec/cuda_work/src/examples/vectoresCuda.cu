...
__global__ void d_procesarVectores(int * arrayOrigen, int * arrayDestino, int tamVector);
...
int main(int argc, char* argv[])
{
    ...
    // se muestran algunos valores del vector
    for(int i=0;i<10;i++){
        cout<<arrayDestino[i]<<' ';
    }
    cout<<endl;
...
void procesarConGPU(int * arrayOrigen, int * arrayDestino, int tamVector)
{
    ...
    cudaMemcpy(d_arrayOrigen, arrayOrigen, sizeof(int)*tamVector, cudaMemcpyHostToDevice);
    ...
    // ha cambiado.
    cudaMemcpy(arrayDestino, d_arrayDestino, sizeof(int)*tamVector, cudaMemcpyDeviceToHost);
